...
const TenantSchema = Yup.object().shape({
  ...
  alias: Yup.string()
    ...
    .required('Required'),
  billingContact: Yup.string().max(255, 'Too Long!').required('Billing contact is required'),
});
...
const TenantForm: React.FC<TenantFormProps> = ({ tenant, onClose }) => {
  ...
  const handleSubmit = async (values: TenantFormValues): Promise<void> => {
    if (tenant) {
      const updateData: UpdateTenantInput = { name: values.name, billingContact: values.billingContact };
